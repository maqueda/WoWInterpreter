...
[English README](README.md) · [英文用户指南](Documentation/WoWInterpreter-2.2.1-User-Guide-English.docx) · [简体中文用户指南](Documentation/WoWInterpreter-2.2.1-User-Guide-Chinese-Simplified.docx)
...
`WoWInterpreter-2.2.1-Setup.exe`
...
`WoWInterpreter.log` 位于已安装的可执行文件旁。日志约达到 5 MiB 时轮转，
并保留三个备份（`.1` 至 `.3`），总量约为 20 MiB。报告传输故障时，请附上
当前日志、必要时的近期备份，以及安装目录 `_internal/Bridge` 中相应的 PNG/TXT
诊断文件对。程序最多保留十组已识别的诊断事件。截图可能包含游戏画面，分享前请检查。
